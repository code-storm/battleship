const TERRITORY = {
  ...
  export function createBattlefield(
    ...
    for (let x = 0; x < shipCoordForPlayerA.length; x++) {
      battleGround[shipCoordForPlayerA[x][0]][
        shipCoordForPlayerA[x][1]
      ].territory = TERRITORY.PLAYER_A;
      ++battleGround[shipCoordForPlayerA[x][0]][shipCoordForPlayerA[x][1]].ships;
  
      battleGround[shipCoordForPlayerB[x][0]][
        shipCoordForPlayerB[x][1]
  ...
  function denoteBlock(block, territoryId, hits) {
    if (block.attacked) {
      if (
        block.territory === TERRITORY.NO_ONE &&
  ...
  export function getCompleteGameString(battleGround, movesA, movesB, missiles) {
    for (let i = 0; i < movesA.length; i++) {
      battleGround[movesA[i][0]][movesA[i][1]].attacked = true;
      battleGround[movesA[i][0]][movesA[i][1]].attackedBy.push(
        TERRITORY.PLAYER_A
      );
      // --battleGround[movesA[i][0]][movesA[i][1]].ships;
  
      battleGround[movesB[i][0]][movesB[i][1]].attacked = true;
      battleGround[movesB[i][0]][movesB[i][1]].attackedBy.push(
        TERRITORY.PLAYER_B
      );
    ...
    if (playerAResponse.loss === playerBResponse.loss) {
      ...
      playerAResponse.loss > playerBResponse.loss
    ) {
